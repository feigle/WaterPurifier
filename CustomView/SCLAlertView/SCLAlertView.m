...
- (id)init
{
    ...
    if (self)
    {
        ...
        _labelTitle.textColor = ColorFromRGB(0x4D4D4D);
        _viewText.textColor = ColorFromRGB(0x4D4D4D);
        _contentView.layer.borderColor = ColorFromRGB(0xCCCCCC).CGColor;
    }
    return self;
}
...
-(SCLAlertViewResponder *)showTitle:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    frame.origin.y = vc.view.frame.origin.y-CGRectGetHeight([UIScreen mainScreen].applicationFrame)/2;
    ...
    [UIView animateWithDuration:0.6f animations:^{
        ...
        [UIView animateWithDuration:0.4f animations:^{
            ...
            //弹跳动画
            [PRTween animationLikeAlertView: self.circleIconImageView];
        }];
    }];
 ...
 *  @param title            标题
 *  @param subTitle         详细描述
 *  @param closeButtonTitle 关闭按钮标题
 *  @param duration         自动关闭时间（设0秒表示手动关闭）
 */
...
{
    [self showTitle:[UIApplication sharedApplication].keyWindow.rootViewController title:title subTitle:subTitle duration:duration completeText:closeButtonTitle style:Info];
}
/**
 * 提示警告，可编辑
 */
