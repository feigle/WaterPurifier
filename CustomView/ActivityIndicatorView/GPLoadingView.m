...
- (void)startAnimation
{
    ...
    self.anglePer = 0;
    dispatch_time_t looptime = dispatch_time(DISPATCH_TIME_NOW, 0.5);
    dispatch_after(looptime, dispatch_get_main_queue(), ^(void){
        [self drawPathAnimation:nil];
    });
...
- (void)drawPathAnimation:(NSTimer *)timer
{
    ...
    [self performSelector:@selector(drawPathAnimation:) withObject:nil afterDelay:0.5];
