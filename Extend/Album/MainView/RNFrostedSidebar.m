...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _imageView.frame = CGRectMake(15, 12, 25, 25);
        [self addSubview:_imageView];
        
        _labelText = [[UILabel alloc] init];
        _labelText.textColor = [UIColor whiteColor];
        _labelText.font = [UIFont systemFontOfSize:18];
        _labelText.frame = CGRectMake(60, 0, 100, 50);
...
- (instancetype)initWithImages:(NSArray *)images title:(NSArray*) aTitle selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors{
    if (self = [super init]) {
        self.indexSelect = 2;
...
//        _itemSize = CGSizeMake(_width/2, _width/2);
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            view.labelText.text = [_arrayTitle objectAtIndex:idx];
...
//    blurImage = [blurImage applyBlurWithRadius:5 tintColor:self.tintColor saturationDeltaFactor:1.8 maskImage:nil];
    blurImage = [blurImage applyBlurWithRadius:5 tintColor:[UIColor colorWithRed:7/255.0 green:8/255.0 blue:10/255.0 alpha:0.9] saturationDeltaFactor:1.8 maskImage:nil];
//    [UIColor colorWithRed:42/255.0 green:45/255.0 blue:51/255.0 alpha:1.0]
...
//        view.layer.borderWidth = self.borderWidth;
        if (self.indexSelect == idx) {
            if (idx == 0) {
                view.labelText.textColor = [UIColor whiteColor];
            }else if(idx == 1){
                view.labelText.textColor = [UIColor colorWithRed:255/255.0 green:71/255.0 blue:255/255.0 alpha:1.0];
            }else if(idx == 2){
                view.labelText.textColor = [UIColor colorWithRed:71/255.0 green:241/255.0 blue:255/255.0 alpha:1.0];
            }else if(idx == 3){
                view.labelText.textColor = [UIColor colorWithRed:77/255.0 green:255/255.0 blue:71/255.0 alpha:1.0];
            }else if(idx == 4){
                view.labelText.textColor = [UIColor colorWithRed:255/255.0 green:238/255.0 blue:71/255.0 alpha:1.0];
            }else if(idx == 5){
                view.labelText.textColor = [UIColor colorWithRed:255/255.0 green:71/255.0 blue:104/255.0 alpha:1.0];
            }else if(idx == 6){
                view.labelText.textColor = [UIColor grayColor];
            }
            view.labelText.font = [UIFont systemFontOfSize:18];

        }else
        {
            view.labelText.textColor = [UIColor whiteColor];
            view.labelText.font = [UIFont systemFontOfSize:16];
        }
...
//        CGRect frame = CGRectMake(leftPadding, topPadding*idx + self.itemSize.height*idx + topPadding, self.itemSize.width , self.itemSize.height);
        int height = 10;
        CGRect frame = CGRectMake(leftPadding, topPadding + height*idx + self.itemSize.height*idx, self.itemSize.width , self.itemSize.height);
        if (idx == 6) {
            frame = CGRectMake(10, topPadding + height*idx + self.itemSize.height*idx + 60, self.itemSize.width , self.itemSize.height);
        }else{
            frame = CGRectMake(leftPadding, topPadding + height*idx + self.itemSize.height*idx, self.itemSize.width , self.itemSize.height);
        }
        view.frame = frame;
        view.layer.cornerRadius = 5.0f;//设置图片的圆角
    }];
    
    NSInteger items = [self.itemViews count];
    int sizeHeight = items * (self.itemSize.height + leftPadding) + leftPadding;
    NSLog(@"sizeHeight = %d",sizeHeight);
    self.contentView.contentSize = CGSizeMake(0, sizeHeight);
}
