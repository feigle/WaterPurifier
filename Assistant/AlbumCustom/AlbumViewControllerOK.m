...
//    [CommonFunc setExtraCellLineHidden:self.AlbumTablewView];
    self.AlbumTablewView.frame = CGRectMake(0, 0, MRScreenWidth, MRScreenHeight-cellHeight);
...
- (void)addPhotos:(NSArray*)photos andCell:(UITableViewCell*)cell indexPath:(NSIndexPath *)indexPath
{
    ...
    NSMutableDictionary *imageViewDic = [[NSMutableDictionary alloc]init];
    if (indexPath.section < [self.imageViewsArr count]) {
        imageViewDic = [self.imageViewsArr objectAtIndex:indexPath.section];
    }
    //每行4张图片
    float imgWidth = (MRScreenWidth-space_w*5)/4;
    for (int i=0; i<[photos count]/4+1; i++) {
        ...
        for (int j=0; j<4; j++) {
            ...
            int vt = i*4+j+conTag;
            UIView *contentViewSmall = (UIView *)[imageViewDic objectForKey:[NSString stringWithFormat:@"%d",vt]];
            if (contentViewSmall) {
                [contentView addSubview:contentViewSmall];
            }else{
                //用于装载图片，按钮，删除图标
                contentViewSmall = [[UIView alloc]initWithFrame:cell.bounds];
...
//                    });
                ...
                [imageViewDic setValue:contentViewSmall forKey:[NSString stringWithFormat:@"%d",contentViewSmall.tag]];
            }
            
        }
    }
    if (indexPath.section>=[self.imageViewsArr count]) {
        [self.imageViewsArr addObject:imageViewDic];
    }
